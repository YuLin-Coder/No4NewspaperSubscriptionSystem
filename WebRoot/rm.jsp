...
function top2(){
   	form3.action="<%=basePath%>rm.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%>rm.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>rm.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>rm.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>rm.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>rm.jsp?page="+page;
      form3.submit();
...
<%   
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from cp order by xl desc",7);
