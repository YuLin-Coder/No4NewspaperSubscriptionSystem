...
<%  String mc=Common.toChineseAndTrim(request.getParameter("mc"));
	String kc=Common.toChineseAndTrim(request.getParameter("kc"));
	String fl=Common.toChineseAndTrim(request.getParameter("fl"));
	String kjg=Common.toChineseAndTrim(request.getParameter("kjg"));
	String ejg=Common.toChineseAndTrim(request.getParameter("ejg"));
	cb.setEVERYPAGENUM(15);

	String sqlmc ="";
	if(mc!=null&& !"".equals(mc)){
        sqlmc=  "and mc like '%"+mc+"%'";
    }

    String sqlkc ="";
    if(kc!=null && !"".equals(kc)){
        sqlkc=  "and kc like '%"+kc+"%'";
    }


	int cou = cb.getMessageCount("select count(*) from cp where fl='"+fl+"'"+sqlkc+sqlmc);//得到信息总数
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from cp where  fl='"+fl+"'"+sqlkc+sqlmc+" order by id desc",7);
...
<%}} %> 
  ...
  <div class="blank"></div>
  <div class="page">
  <form action="" method="post" name="form3"><input type="hidden" name="mc" value="<%=mc%>" /><input type="hidden" name="fl" value="<%=fl%>" />
  <input type="hidden" name="kjg" value="<%=kjg%>" />
  <input type="hidden" name="ejg" value="<%=ejg%>" />
